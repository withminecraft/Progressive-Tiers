...
public class SpecialCombatEvents {
    ...
    @SubscribeEvent
    public static void onWitherSkeletonAttack(LivingIncomingDamageEvent event) {
    ...
    @SubscribeEvent
    public static void onHuskAttack(LivingIncomingDamageEvent event) {
        Entity attacker = event.getSource().getEntity();
        ...
        if (attacker instanceof LivingEntity livingAttacker) {
            if (livingAttacker.getPersistentData().getBoolean(HuskSpecial.TAG_DROP_ANCIENT_LOOT)) {
                ...
                String cooldownTag = "last_sandstorm_time";
                long lastTime = livingAttacker.getPersistentData().getLong(cooldownTag);

                if (currentTime - lastTime >= 20) { 
                    CataclysmCompat.spawnSandstorm(livingAttacker);
                    livingAttacker.getPersistentData().putLong(cooldownTag, currentTime);
                }
            }
        }
    }
    ...
    @SubscribeEvent
    public static void onSpiderAttack(LivingIncomingDamageEvent event) {
        Entity attacker = event.getSource().getEntity();
        LivingEntity target = event.getEntity();
        ...
        if (attacker instanceof LivingEntity livingAttacker && 
            ...
            if (target != null && !target.level().isClientSide) {
                ...
                if (target.level().isEmptyBlock(pos)) {
                    target.level().setBlockAndUpdate(pos, Blocks.COBWEB.defaultBlockState());
                }
            }
        }
    }
    ...
    @SubscribeEvent
    public static void onTridentHit(LivingIncomingDamageEvent event) {
    ...
    public static void onMountHurt(LivingIncomingDamageEvent event) {
    ...
    public static void onSpecialDrops(LivingDropsEvent event) {
        ...
        if (entity.getPersistentData().getBoolean(HuskSpecial.TAG_DROP_ANCIENT_LOOT)) {
            Item ancientMetal = BuiltInRegistries.ITEM.get(ResourceLocation.parse("cataclysm:ancient_metal_ingot"));
            if (ancientMetal != Items.AIR) {
                event.getDrops().add(new ItemEntity(entity.level(), entity.getX(), entity.getY(), entity.getZ(), 
                    new ItemStack(ancientMetal, 3)));
            }

            Item kobolediatorSkull = BuiltInRegistries.ITEM.get(ResourceLocation.parse("cataclysm:kobolediator_skull"));
            if (kobolediatorSkull != Items.AIR) {
                event.getDrops().add(new ItemEntity(entity.level(), entity.getX(), entity.getY(), entity.getZ(), 
