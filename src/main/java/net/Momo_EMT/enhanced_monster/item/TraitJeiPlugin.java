...
public class TraitJeiPlugin implements IModPlugin {
    ...
    public ResourceLocation getPluginUid() {
        return ResourceLocation.tryBuild(EnhancedMonster.MODID, "main");
    }
        ...
        for (String traitId : TraitConfig.getValidTraits()) {
            ...
            for (int lvl = 0; lvl < maxLvl; lvl++) {
                ...
                if (!left.isEmpty() && !result.isEmpty()) {
                    anvilRecipes.add(factory.createAnvilRecipe(left, Collections.singletonList(right), Collections.singletonList(result)));
                }
            }
